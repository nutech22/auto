...
cl = treetc.LINE()
#cl.login(qr=True) #นุ๊ย#
cl.login(token="EpEzpxlB7sh9YTwBvm2d.WFY52Ak0NqvAPMx4kb5rVq.716R8EfaSyfKn8AAzpT+cBRXjVB0vhX5bsbMbNutWp8=")
cl.loginResult()
kt = ks = ki = kk = kc = cl
print "Login ™Ŧяәәƅoŧ"
reload(sys)
...
#-------------------------------------------------------------------------#
        if op.type == 17:
            ...
            cl.sendText(op.param1,"ยินดีต้อนรับ "+cl.getContact(op.param2).displayName +"\nเข้าสู่ กลุ่ม"+ str(ginfo.name)+ ", สวัสดี จร้า ขอบคุณ น๊าา ที่เข้ามาเป็นเพื่อนกันในกลุ่ม \n(*´･ω･*)")
            print "สมาชิกเข้าร่วม"
	if op.type == 15:
            random.choice(KAC).sendText(op.param1, cl.getContact(op.param2).displayName + ", ขอให้โชคดี มีโอกาสดีๆๆ แล้วพบกันใหม่น๊า \n(*´･ω･*)")
